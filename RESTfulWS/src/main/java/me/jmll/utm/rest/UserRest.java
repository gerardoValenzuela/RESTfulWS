...
public class UserRest {
	...
	public Map<String, Object> getUsersJSON() {
		...
		for(int i = 0; i < users.size(); i++) {
			...
			links.add(new Link(builder.path("/user/" + user.getUsername()).build().toString(), user.getUsername()));
		}
	...
	public UserLinkListResource getUsersXML() {
		...
		for(int i = 0; i < users.size(); i++) {
			User user = users.get(i);
			userLinksResource.addUserLink(new Link(builder.path("/user/" + user.getUsername()).build().toString(), user.getUsername()));
		}
		// }
	...
	public Map<String, Object> getUserJSON(@PathVariable("username") String username) {
		...
		links.add(new Link(builder.path("/user/" + username).build().toString(), "self"));
		Map<String, Object> response = new Hashtable<>(1);
		response.put("_links", links);
	...
	public UserResource getUserXML(@PathVariable("username") String username) {
		...
		resource.addLink(new Link(builder.path("/user/" + username).build().toString(), "self"));
